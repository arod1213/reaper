...
class MediaItem():
  ...
  def __init__(self, item):
    ...
    self.take = rp.RPR_GetMediaItemTake(self.id, int(rp.RPR_GetMediaItemInfo_Value(self.id, "I_CURTAKE")))
  ...
  def getToLoud(self, type, target):
    self.target = rp.RPR_CalculateNormalization(self.source, type, target, self.startoffset, self.startoffset+(self.length / self.playrate))
    return self.target
  ...
  for x in range(0, rp.RPR_CountSelectedMediaItems(0)):
    ...
    if len(volChange) > 1: # needs at least 2 to run statistics
      ...
      if abs(z) < tolerance:
        ...
        item.changeVol(ampDiffLinear(item.vol, item.getToLoud(type, target), scale * .5)) # cut change in half
...
userInput = rp.RPR_GetUserInputs("Clip Gain", 3, "Type:,Peak:,Strength (0-1):", "RMS,-27, 1", 20)[4]
