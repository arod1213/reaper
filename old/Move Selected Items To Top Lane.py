...
class MediaItem():
  def __init__(self, item):
    ...
    self.parTrack = RPR_GetMediaItemInfo_Value(self.id, "P_TRACK")
    self.start = RPR_GetMediaItemInfo_Value(self.id, "D_POSITION")
    self.length = RPR_GetMediaItemInfo_Value(self.id, "D_LENGTH")
    self.end = self.start + self.length
    self.fixedlane = RPR_GetMediaItemInfo_Value(self.id, "I_FIXEDLANE")
  
  def checkTrackSel(self):
    x = RPR_GetMediaTrackInfo_Value(self.parTrack, "I_SELECTED")
    if x == 1:
      return True
    else:
      return False
  ...
  def setRazor(self):
    ...
    console(bounds)
    console(RPR_GetSetMediaTrackInfo_String(self.parTrack, "P_RAZOREDITS", bounds, True)[2])
...
def main():
  ...
  for i in range(0,len(items)):
    ...
    RPR_MoveEditCursor(items[i].start - editCursor, False) # move edit cursor to item
    
    setItemSelect(items, False) # deselect all items
    items[i].select(True)
    RPR_Main_OnCommand(40068,0) # move item to top lane
    items[i].setRazor()
main()

#console(RPR_GetSetMediaTrackInfo_String(RPR_GetSelectedTrack(0,0), "P_RAZOREDITS", "", False)[3])
RPR_UpdateArrange()
