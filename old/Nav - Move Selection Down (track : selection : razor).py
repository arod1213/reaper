...
def moveTrackSelection():
  ...
  lastSelTrack = MediaTrack(RPR_GetSelectedTrack(0, selTrackCount-1))

  if lastSelTrack.int < trackCount:
   RPR_SetOnlyTrackSelected(RPR_GetTrack(0, lastSelTrack.int))
...
def moveSelection():
  ...
  index = 0
  track = MediaTrack(RPR_GetTrack(0,index)) # get first track

  while len(RPR_GetSetMediaTrackInfo_String(track.id, "P_RAZOREDITS", "", False)[3]) == 0 and index < numTracks:
    index += 1
  ...
  if len(track.razorBounds) > 0:
    if index < numTracks-1: # as long as not top track
      ...
      trackBelow = MediaTrack(RPR_GetTrack(0, index+1))
      
      if track.lanestate == 0:
        RPR_GetSetMediaTrackInfo_String(trackBelow.id, "P_RAZOREDITS", track.razorBounds, True) # set razor edit up a track
      ...
      RPR_SetOnlyTrackSelected(trackBelow.id) # move track selection to one above bottom razor edit
